-- Performance optimization indexes for feed loading
-- This migration adds indexes to improve query performance for the feed

-- Index for posts query with status and type filtering
CREATE INDEX IF NOT EXISTS idx_posts_status_type_created_at 
ON posts (status, type, created_at DESC) 
WHERE student_id IS NOT NULL;

-- Index for posts with student_id and created_at for efficient ordering
CREATE INDEX IF NOT EXISTS idx_posts_student_created_at 
ON posts (student_id, created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_student_followers_follower 
ON student_followers (follower_user_id);

-- Index for student_followers with student_id for follower count queries
CREATE INDEX IF NOT EXISTS idx_student_followers_student 
ON student_followers (student_id);

-- Composite index for student_followers with both user and student for follow status checks
CREATE INDEX IF NOT EXISTS idx_student_followers_user_student 
ON student_followers (follower_user_id, student_id);

-- Index for students with user_id for efficient joins
CREATE INDEX IF NOT EXISTS idx_students_user_id 
ON students (user_id);

-- Index for users with role for role-based queries
CREATE INDEX IF NOT EXISTS idx_users_role 
ON users (role);

-- Index for posts with media_url for content filtering
CREATE INDEX IF NOT EXISTS idx_posts_media_url 
ON posts (media_url) 
WHERE media_url IS NOT NULL AND media_url != '';

-- Index for posts with caption for content filtering
CREATE INDEX IF NOT EXISTS idx_posts_caption 
ON posts (caption) 
WHERE caption IS NOT NULL AND caption != '';

-- Partial index for non-processing posts (most common query)
CREATE INDEX IF NOT EXISTS idx_posts_ready_posts 
ON posts (created_at DESC, student_id) 
WHERE (status != 'processing' OR status IS NULL) 
  AND student_id IS NOT NULL 
  AND (type != 'announcement' OR type IS NULL);

...
export function useAuth() {
  ...
  useEffect(() => {
    ...
    const handleCustomStorageChange = async () => {
      // Check if we're logging out (no token means logout)
      const token = localStorage.getItem("token");
      if (!token) {
        // User is logging out - clear React state immediately
        setUser(null);
        ...
        // User is logging in or auth changed - fetch fresh user data
        await fetchUser();

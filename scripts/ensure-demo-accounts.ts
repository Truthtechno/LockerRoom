...
/**
 * Comprehensive Idempotent Demo Account Seeder for LockerRoom
 * 
 * This script safely creates all required demo accounts without deleting any existing data.
 * It uses idempotent operations to ensure accounts are only created if missing.
 */
...
async function ensureSchoolAdminExists() {
  ...
  console.log("  ✅ School admin user ensured");
  return { userId: user[0].id, schoolId, adminProfileId };
}
...
async function ensurePublicViewerExists() {
  ...
  // Update the viewer record to link to the user
  await sql`
    UPDATE viewers SET user_id = ${user[0].id} WHERE id = ${viewerProfileId}
  `;
  
  console.log("  ✅ Public viewer user ensured");
...
async function ensureScoutExists() {
  ...
  const user = await sql`
    ...
    ON CONFLICT (email) DO UPDATE SET
      ...
      role = EXCLUDED.role,
      name = EXCLUDED.name,
      xen_id = EXCLUDED.xen_id,
      otp = EXCLUDED.otp,
      is_one_time_password = EXCLUDED.is_one_time_password
    RETURNING id
  `;
  ...
  if (user.length > 0) {
    userId = user[0].id;
    console.log("  ✅ Scout user ensured");
  } else {
  ...
  // Create scout admin record (for admin authentication flow)
  ...
  console.log("  ✅ Scout admin record ensured");
  return userId;
}

async function main() {
  try {
    console.log("🌱 Starting comprehensive demo account seeding...\n");
